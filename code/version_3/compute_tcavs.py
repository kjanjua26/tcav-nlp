...
def compute_word_tcav(concept_cavs, random_cavs,
    ...
    for ix in range(1, num_layers+1):
        ...
        for concept, cavs_of_runs in layer_cavs.items():
            ...
            print(f"[INFO] Layer - {ix} TCAVs - {tcavs_per_run}.")
...
def run_for_chosen_word_write_to_pickle(sentences, concept_cavs, random_cavs,
    ...
    to_check_concepts = ["NN", "JJ", "NNS", "JJR", "JJS", "DT", "CC", "CD", "VB", "VBP"]

    for concept in list(sentences.keys()):
        if concept not in to_check_concepts:
            del sentences[concept]
    ...
    result = pool.map(lambda i: compute_word_tcav(concept_cavs, random_cavs,
                                                    ...
                                                    num_layers, word,
                                                    num_of_runs, model_type,
                                                    to_check_concepts[i], if_rand), range(len(to_check_concepts)))

    for concept_masked, word_layer_wise_tcavs in enumerate(result):
